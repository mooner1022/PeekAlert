...
package dev.mooner.peekalert.interpolator
...
open class CubicBezierInterpolator(start: PointF, end: PointF) : Interpolator {
    ...
    init {
        require(!(start.x < 0 || start.x > 1)) { "startX value must be in range 0..1" }
        require(!(end.x < 0 || end.x > 1)) { "endX value must be in range 0..1" }
        this.start = start
        this.end = end
    }
